...
3. **Close GitHub issue**: If a GitHub issue exists (check `changes/<id>/proposal.md` for issue reference):
   - Update label to `openspec-deployed` via `gh issue edit <number> --add-label "openspec-deployed" --remove-label "openspec-implementing"`
   - Close issue via `gh issue close <number> --comment "Deployed via openspec archive on $(date +%Y-%m-%d)"`
