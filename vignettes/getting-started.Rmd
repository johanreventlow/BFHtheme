...
| Export figures | `bfh_save()`, `get_bfh_dimensions()` |
...
Remove gridlines and axis accents with `theme_bfh_minimal()` whenever you want
the data ink to stand out.
...
`theme_bfh_print()` increases contrast, line weights, and title emphasis so
graphics survive grayscale printing.
...
`theme_bfh_presentation()` scales typography and legends for projector use.
...
Use `theme_bfh_dark()` when presenting on dark backgrounds or embedding plots
in night-mode dashboards.
